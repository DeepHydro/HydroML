# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 19:16:13 2019
...
########## training #########################
M = model_svr.fit(train_x,train_y)
...
############### save image   #####################
save_path=os.path.join('SVR%s.png' %dt.datetime.now().strftime('%Y%m%d%H%M%S'))
...
Y_r = (Y - scaler1.inverse_transform(np.array(model_svr.predict(x_r)).reshape(-1,1)))/Y
...
Y_p =  (Y - scaler1.inverse_transform(np.array(model_svr.predict(x_p)).reshape(-1,1)))/Y
...
Y_t =  (Y - scaler1.inverse_transform(np.array(model_svr.predict(x_t)).reshape(-1,1)))/Y
...
Y_w = (Y - scaler1.inverse_transform(np.array(model_svr.predict(x_w)).reshape(-1,1)))/Y
